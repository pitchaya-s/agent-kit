...
usage() {
  cat <<'USAGE'
Usage: bash scripts/install.sh --agent <codex|claude|opencode|gemini|copilot|cursor|shared|all>
       bash scripts/install.sh --target <directory>
USAGE
}
...
# Preflight every destination before modifying anything.
for target in "${TARGETS[@]}"; do
  for skill in "${SKILLS[@]}"; do
    ...
    if [ -L "$dest" ]; then
      ...
      echo "error: $dest already points somewhere else" >&2
      exit 1
    fi

    if [ -e "$dest" ]; then
      echo "error: $dest already exists and will not be overwritten" >&2
      exit 1
    fi
  done
done
